...
def analyze(r0,r1_list,filepath_output):
    ...
    for r1 in r1_list:
        ...
        O = gillespie_optimized_fraction([G,r0,r1,MAX_TSTEP,NUM_SAMPLES,init0])
...
if __name__ == '__main__':
    ...
    init0    = inputs_dct['init0']  # 0.0005 # Fraction of disrupted nodes on the network as initial condition (default 0.05)
    r0_list = inputs_dct["r0_list"]
    r1_list = inputs_dct["r1_list"]

    if name_topology == "random":
        prob_er = 0.5
        r0_list = r0_list/prob_er
        G  = load_topology(name_topology,NUM_NODES, prob_er=prob_er)
        filepath_output = make_dirs(f"./Output_OAD/{name_topology}_prob_{prob_er:.3f}_nodes_{NUM_NODES}_samples_{NUM_SAMPLES}_maxtime_{MAX_TSTEP}")
