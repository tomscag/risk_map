...
def gillespie_optimized(argv):
    ...
    nodelist   = argv[1]  # Nodes to remove
    dynp_r0    = argv[2]   # Local spreading rate 
    dynp_r1    = argv[3]   # Field spreading rate

    dynp_tmax  = argv[4]  #'Maximum time steps (it stops if the absorbing state is reached): ' es 1000000
...
def gillespie_optimized_fraction(argv):
    ...
    for sam in range(1,num_sam+1):
        ...
        dyn_NR  = 0 #new
         ...
         # Sort vertices and apply the initial condition
        ver_i_list = [] 
        for ver in np.random.permutation(nw.nodes()): # np.random.permutation(10) = array([1, 7, 4, 3, 0, 9, 2, 5, 8, 6])
            ver_i_list.append(ver)
            dyn_VI[dyn_NI] = ver
            dyn_NI += 1 # total infected
            dyn_sig[ver]= 1 # sigma = 1 for Infected, sigma = 0 for Susceptible
            dyn_NIk += nw.degree(ver) # somma dei degree degli infetti ? N_IS ?
        ...
        while dyn_t <= dynp_tmax and dyn_NI > 0 and dyn_dt>=0:
            ...
            # A = (dynp_r1 * dyn_NI) * (net_N-1) # net_N-1 to include phantom process  
            # A = dynp_r1 * dyn_NI * dyn_NIk   # (dyn_NIk is wrong, should not depend on the topology!)
            dyn_R = M + L + A
        ...
        # gcc = max([len(item) for item in nx.connected_components(nf)])/net_N
        # print(dyn_NS)
        res_sam.append(dyn_NS/net_N)  # Save result here

    
    avg = sum(res_sam)/len(res_sam)
